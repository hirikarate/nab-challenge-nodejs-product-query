...
const debug: debug.IDebugger = require('debug')('nab:repo:user')
...
export class UserRepository extends p.PgCrudRepositoryBase<UserORM, User> implements IUserRepository {
	 ...
	 * @override
	 */
	private _buildPageActiveQuery(query: QueryBuilder<UserORM>, opts: p.RepositoryPageOptions): p.QueryCallbackReturn {
		const q = super.$buildPageQuery(query, opts) as QueryBuilder<UserORM>
		void q.where('status', UserStatus.ACTIVE)
		return q
	}
}
